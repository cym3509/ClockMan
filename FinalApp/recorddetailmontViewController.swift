//
//  recorddetailmontViewController.swift
//  FinalApp
//
//  Created by 米娜 on 2017/7/27.
...
class recorddetailmontViewController: UIViewController {
...
print(chooseuid)
       ...
       // overtimelabel.text = "1"
        ...
        ref.child("Members").child(chooseuid).observeSingleEvent(of: .value , with:  { (snapshot) in
            // print(snapshot)
            if let dataDict = snapshot.value as? [String: AnyObject]{
                print("dataDict=\(dataDict)")
                if let value = snapshot.value as? NSDictionary {
                    let role = value["baseSalary"] as? String
                    if role == RegisterMode.FT.rawValue, let bs = value["baseSalary"] as? String, let salary = Int(bs) {
        ...
        self.ref.child("DayOff").observe(.childAdded, with: { (snapshot) in
            ...
            if let dataDict = snapshot.value as? [String: AnyObject]{
                ...
                if (uid == self.chooseuid) && (splityearmonth[0] == startYear) && (splityearmonth[1] == startMonth){
        ...
        self.ref.child("Salary").child(self.chooseuid).child(splityearmonth[0]).child(splityearmonth[1]).observeSingleEvent(of: .value , with:  { (snapshot) in
        ...
        self.ref.child("Realhourtime").child(self.chooseuid).child(splityearmonth[0]).child(splityearmonth[1]).observe(.childAdded, with: {(snapshot) in
            print(snapshot)
    ...
    @IBAction func backrecord(_ sender: Any) {
        
        
        let Storyboard = UIStoryboard(name: "Main", bundle: nil)
        let dest = Storyboard.instantiateViewController(withIdentifier: "member") as! recordMemberViewController
