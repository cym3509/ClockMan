//
//  MembermenuTableViewController.swift
//  FinalApp
//
//  Created by 米娜 on 2017/7/17.
...
class MembermenuTableViewController: UITableViewController {
    ...
    @IBOutlet weak var changeCell: memberchangecountTableViewCell!
                    ...
                    if let dic = snapshot.value as? [String: AnyObject]{
                        ...
                        self.ref.child("Members").child(uid!).observeSingleEvent(of: .value, with: {(snapshot) in
                            ...
                            if let dataDict2 = snapshot.value as? [String: AnyObject]{
                                ...
                                if(company == companyNum) && (changemember == name ) && (changeauditText == "nil"){
                                    ...
                                    self.changeCell.count.setTitle("\(changecount)", for: .normal)
                                    self.changeCell.count.backgroundColor = UIColor.red
                                    self.changeCell.count.layer.borderColor = UIColor.red.cgColor
