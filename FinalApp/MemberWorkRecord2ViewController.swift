//
//  MemberWorkRecord2ViewController.swift
//  FinalApp
//
...
import UIKit
import Firebase
class MemberWorkRecord2ViewController: UIViewController ,UITableViewDelegate, UITableViewDataSource{
    ...
    func load(){
        ...
        ref.child("Members").child(uid!).observeSingleEvent(of: .value, with:  { (snapshot) in
            if let value = snapshot.value as? [String: String]{
                ...
                if let bs = value["baseSalary"], let salary = Int(bs) {
                    self.baseSalary = salary
                }
                if value["role"] == RegisterMode.FT.rawValue {
                    self.roleType = .FT
                }
    ...
    @IBAction func chooseym(_ sender: Any) {
        ...
        let dest = Storyboard.instantiateViewController(withIdentifier: "MemberWorkRecord2ViewController") as!  MemberWorkRecord2ViewController
